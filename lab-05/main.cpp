...
public:
    ...
    {
        std::cout << (std::string(std::string("./data/") + filename)).c_str() << std::endl;
    ...
    int compress(const char* filename, const ll Filesize)
    {
        int my_cnt = 0;
...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        std::cout << "Usage: ./a.exe [mode] [file_name]"; 
        return -1;
    }
    ...
    if (option == "c") {
        ...
        int comp_filesize = huf.compress(filename_c, filesize);
